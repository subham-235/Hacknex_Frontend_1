        ...
        <>
          <div className="responder-main">
            ...
            >
              ...
              <p className="responder-kicker">YOUR AVAILABILITY</p>
              <h2>
                {available
                  ? 'You can receive nearby requests.'
                  : 'A small step. A chance to help.'}
              </h2>
            ...
            >
              ...
              {query.data?.map((r) => (
                ...
                >
                  <h3>
                    {r.assigned
                      ? 'You are the assigned responder'
                      : 'Emergency nearby'}
                  </h3>
                  <Status
                    value={
                      r.assigned
                        ? r.responder?.currentStatus || 'assigned'
                        : r.request?.status || r.status
                    }
                  />
                  <p>
                    Approximately {r.request?.distanceAtNotification} m away
                    when notified.
                  </p>
...
export function OwnerCoordination({
  ...
  return (
    <article className="card">
      ...
      <p>
        Severity: {session.severity} · Distress confidence:{' '}
        {session.distressConfidence ?? '—'}%
      </p>
      <p>
        {session.nearbyResponderRequests?.length || 0} nearby responders
        notified · Radius stage {session.escalationStage || 0}
      </p>
      <p>
        {session.activeResponder
          ? `Responder ${session.activeResponder.userId} · ${session.activeResponder.currentStatus.replaceAll('_', ' ')}`
          : 'Waiting for confirmed help'}
      </p>
      <Status value={session.escalationState || 'searching'} />
      <Distance tracking={session.responderTracking || undefined} />
      {session.latestVictimLocation && (
        <p>
          {session.latestVictimLocation.fresh
            ? 'Current victim location'
            : 'Last known victim location'}
          : {session.latestVictimLocation.latitude},{' '}
          {session.latestVictimLocation.longitude}
          <br />
          <small>
            Observed{' '}
            {new Date(
              session.latestVictimLocation.observedAt || '',
            ).toLocaleString()}
          </small>
        </p>
      )}
      {session.geoRiskSignals?.length ? (
        <p>{session.geoRiskSignals.join(' · ').replaceAll('_', ' ')}</p>
      ) : null}
      {!demo && active && (
        <p role="status">
          {locationStatus} Location updates continue when you switch screens.
